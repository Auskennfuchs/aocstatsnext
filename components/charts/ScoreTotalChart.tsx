...
const ScoreTotalChart = () => {
    ...
    const { filteredMembers, stats: { members, memberColors, maxDays } } = useAoCStats()
        ...
        <>
            ...
            <AreaChart width={containerWidth} height={45 * members.length} data={calcData}>
                ...
                {sortedMembers.map(({ name }) => (
                    <Area key={name} type="monotone" dataKey={name} stackId={name} stroke={memberColors[name]} fill={memberColors[name]} fillOpacity={markedMember === name ? 0.5 : 0.03} dot
